...
function Form() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title || !amount || !date || !category) {
      setError("Please fill in all required fields!"); // Use setError directly
      return;
    }

    if (isNaN(Number(amount)) || Number(amount) <= 0) {
      setError("Amount must be a positive number!"); // Use setError directly
      return;
    }
    ...
    const payload = {
      ...
      date: date?.toISOString(), // Ensure date is sent in ISO string format
      category,
      description,
    };
